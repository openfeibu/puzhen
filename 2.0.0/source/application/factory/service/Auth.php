...
use app\factory\model\factory\Access;
use think\Session;
use app\factory\model\factory\User;
use app\factory\model\factory\UserRole;
use app\factory\model\factory\RoleAccess;
...
class Auth
{
    ...
    {
        // 商家登录信息
        $this->factory = Session::get('fbshop_factory');
        // 当前用户信息
        if (!empty($this->factory)) {
            $this->user = User::detail($this->factory['user']['factory_user_id']);
        }
    }
    ...
    private function getAccessUrls()
    {
        if (empty($this->accessUrls)) {
            // 获取当前用户的角色集
            $roleIds = UserRole::getRoleIds($this->user['factory_user_id']);
            // 根据已分配的权限
