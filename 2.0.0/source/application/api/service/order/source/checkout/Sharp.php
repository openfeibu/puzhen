    ...
    public function validateLimitNum()
    {
        foreach ($this->goodsList as $goods) {
            ...
            if ($alreadyBuyNum == 0 && $goods['total_num'] > $goods['limit_num']) {
                $this->error = "很抱歉，该产品限购{$goods['limit_num']}件，请修改购买数量";
                return false;
            }
            // 情况2: 已购买3件, 实际想购买1件
            if ($alreadyBuyNum >= $goods['limit_num']) {
                $this->error = "很抱歉，该产品限购{$goods['limit_num']}件，您当前已下单{$alreadyBuyNum}件，无法购买";
                return false;
            }
            // 情况3: 已购买2件, 实际想购买2件
            if (($alreadyBuyNum + $goods['total_num']) > $goods['limit_num']) {
                $diffNum = ($alreadyBuyNum + $goods['total_num']) - $goods['limit_num'];
                $this->error = "很抱歉，该产品限购{$goods['limit_num']}件，您最多能再购买{$diffNum}件";
                return false;
            }
        }
        return true;
