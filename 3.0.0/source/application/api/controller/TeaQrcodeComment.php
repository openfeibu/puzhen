...
class TeaQrcodeComment extends Controller
{
     ...
     */
    public function lists($tea_qrcode_id,$goods_id)
    {
        $model = new TeaQrcodeCommentModel;
        $list = $model->getCommentList($tea_qrcode_id,$goods_id);
     ...
     * @throws \think\exception\DbException
     */
    public function add($tea_qrcode_id,$goods_id)
    {
        $model = new TeaQrcodeCommentModel;
        if ($model->add($this->getUser(), $tea_qrcode_id, $goods_id, $this->request->post())) {
